#Binary trees二叉树

* [Binary trees](#bintrees)
  * [基本f:mirror /size /leaves /heigh](#bintreefns)
  * [进阶f:Directions / addresses / paths](#bintreeaddr)
  * [Traversals遍历](#traversals)
  * [Inverting traversals (生成树)](#gentree)
  * [用induction证明BT](#bintreepf)
  
<a name="bintrees"></a>
# Binary trees
简写为BT
一个装着 a 类型数据的 binary tree（二叉树）有两种可能：

* 它是空的，什么都没有；

* 它不是空的，那它就有：
  
  *一个 root（根节点）
  
   root 里面存着一个类型为 a 的 element（元素）
  
   root 左边接着一棵 binary tree，叫 left subtree（左子树）当然，通过递归可以直到左子树可以为空
  
   root 右边接着一棵 binary tree，叫 right subtree（右子树）


```hs
data BT a = Empty | Fork a (BT a) (BT a)
```
`BT a`是类型, BT是类型构造器

Empty是无参数据构造器

Fork 是有参数据构造器(也叫function，用于构造BT a类型的数据）接受三个参数，一个a和两个(BT a)，分别代表root，左右子树
```hs
Fork:: a->(BT a)->(BT a)   ->   (BT a)
```
* 约定 empty trees叫做`Empty` 画成 dangling leaves。
  ```
                              |
  ```


* 给定两个 trees `l` 和 `r`，以及一个 element `x::a`，我们有一个 new tree：
  ```
                              x
                             / \
                            /   \
                           l     r
  ```
  写作 `Fork x l r`。

例如，tree：
```
                             8
                            / \
                           /   \
                          4     16
                         / \   / \
                        2        20
                       / \      /  \
```
用这个 notation 写作：
```haskell
bt1 = Fork 8 (Fork 4 (Fork 2 Empty Empty) Empty) (Fork 16 Empty (Fork 20 Empty Empty))
       root↑ 【              左子树             】【             右子树                 】
```

我们可以像前面一样让 Haskell 通过 deriving 帮我们做一些工作：

```haskell
data BT a = Empty    --不用在意换行，换不换都不影响
          | Fork a (BT a) (BT a) deriving (Show, Read, Eq, Ord)
```

我们有：
```hs
   Empty :: BT a
   Fork  :: a -> BT a -> BT a -> BT a
```

**Puzzle**. Automatically derived 的 `Show`、`Read` 和 `Eq` 做什么应该比较清楚。但是你觉得用 `Ord` derived 出来的 trees 上的 order 应该是什么answer：The automatically derived Ord instance orders binary trees lexicographically according to the order of the constructors and their fields.

<a name="bintreefns"></a>
# 基本 functions on binary trees
## mirror
比如从上面的 tree 得到：（左右翻转）
```
                             8
                            / \
                           /   \
                          16    4
                         / \   / \
                        20        2
                       / \       / \
```
```haskell
mirror :: BT a -> BT a
mirror Empty = Empty
mirror (Fork x l r) = Fork x 【(mirror r) (mirror l)】
```
在上面的 example 上运行后得到：
```hs
mirror bt1 = Fork 8 (Fork 16 (Fork 20 Empty Empty) Empty) (Fork 4 Empty (Fork 2 Empty Empty))
```
上面tree notation 不太适合可视化，适合计算
## size / leaves / height

*size* : nodes 总数(root不算入)：
```haskell
size :: BT a -> Integer
size Empty        = 0
size (Fork x l r) = 1 + size l + size r --从递归的视角看这行，只处理当下的root，然后用递归去处理tail
```
*nodes* 的数量(size)等于 leaves 的数量减一：
```haskell
leaves Empty        = 1 --其他的和size一样
```
我们把 tree 的 *height* 定义成从 root 出发的最深path的length，【用node数量来衡量】：
```haskell
height :: BT a -> Integer
height Empty        = 0
height (Fork x l r) = 1 + max (height l) (height r) --可以自己理解为每次都二选一然后把当前root变成1从而累加
```
一个 balanced binary tree 的 height 大约是它 size 的 log；而一个非常 unbalanced 的 binary tree，比如：
```
                            20
                           / \
                          16
                         / \
                        8
                       / \
                      4
                     / \
                    2
                   / \
```
```haskell
btleft = Fork 20 (Fork 16 (Fork 8 (Fork 4 (Fork 2 Empty Empty) Empty) Empty) Empty) Empty
```
 height = size。

<a name="bintreeaddr"></a>
# Directions, addresses and paths in binary trees

为了从BT里选出某个 subtree，我们会连续向 left 或！right 走，直到找到它

###Address为Direction的list，如[R,R,L,R]
```haskell
data Direction = L | R
                 deriving (Show)
type Address   = [Direction]  --起别名

subtree :: Address -> BT a -> Maybe(BT a)
subtree []     t            = Just t
subtree (_:_)  Empty        = Nothing
subtree (L:ds) (Fork _ l _) = subtree ds l  --递归思想，1先复制f,然后改参数,L:ds变【ds】, Fork...变左子树【l】
subtree (R:ds) (Fork _ _ r) = subtree ds r  --2处理当下的节点，这里直接丢弃所以=不写
```
如果想返回值，那就把第二行改为：（记得改定义）
```
subtree []     (Fork a _ _) = Just a
```
沿用上面的 pattern，定义f，检查一个 address 在给定 tree 里是否 valid：
```haskell
isValid :: Address -> BT a -> Bool
isValid []     _            = True
isValid (_:_)  Empty        = False
isValid (L:ds) (Fork _ l _) = isValid ds l
isValid (R:ds) (Fork _ _ r) = isValid ds r
```
输入一棵 binary tree，输出所有 valid address 的 list
```haskell
validAddresses :: BT a -> [Address]
validAddresses Empty        = [[]]
validAddresses (Fork _ l r) = [[]]
                           ++ [L:ds | ds <- validAddresses l]
                           ++ [R:ds | ds <- validAddresses r]
```
```
[]
[L]
[R]
[L, R]
```
List comprehensions 总是可以被消除。在这个 example 中，它们会变成：
```haskell
validAddresses' :: BT a -> [Address]
validAddresses' Empty        = [[]]
validAddresses' (Fork _ l r) = [[]]
                            ++ (map (L:) (validAddresses' l))
                            ++ (map (R:) (validAddresses' r))
```
我们期待：
```hs
    isValid ds t = ds `elem` (validAddresses t)
```
一个 address 合不合法，就看它在不在 validAddresses 生成出来的 list 里面。
在里面，就是 valid。

所有从 root 到 leaf 的 paths 的 list 也有类似的 definition：
```haskell
btpaths :: BT a -> [[a]]
btpaths Empty        = [[]]
btpaths (Fork x l r) = [x:xs | xs <- btpaths l]
                    ++ [x:xs | xs <- btpaths r]
```


<a name="traversals"></a>
# Traversals遍历

定义 standard in-order和 pre-order
```haskell
treeInOrder :: BT a -> [a]
treeInOrder Empty         = []
treeInOrder (Fork x l r)  = treeInOrder l ++ [x] ++ treeInOrder r

treePreOrder :: BT a -> [a]
treePreOrder Empty        = []
treePreOrder (Fork x l r) = [x] ++ treePreOrder l ++ treePreOrder r
```
用bt1为例
```
                             8
                            / \
                           /   \
                          4     16
                         / \   / \
                        2         20
                       / \        /  \
```
我们得到：
```hs
> treeInOrder bt1
[2,4,8,16,20]

> treePreOder bt1
[8,4,2,16,20]
```
而对于 `btleft`：
```
                            20
                           / \
                          16
                         / \
        btleft =        8
                       / \
                      4
                     / \
                    2
                   / \
```
```hs
inOrder [2,4,8,16,20]
preOrder[20,16,8,4,2]
```
---------
### hard
[BFS的遍历]更 tricky。我们先定义一个 function，它接收一个 tree，然后产生一个 lists of lists：第一个 list 是 level zero 的 nodes，也就是 root；然后是 level one 的 nodes，也就是 root 的 successors；然后是 level two 的 nodes，以此类推：
```haskell
levels :: BT a -> [[a]]
levels Empty        = []
levels (Fork x l r) = [[x]] ++ zipappend (levels l) (levels r)
  where
    zipappend []       yss      = yss
    zipappend xss      []       = xss
    zipappend (xs:xss) (ys:yss) = (xs ++ ys) : zipappend xss yss
```
```hs
> levels btexample
[[8],[4,16],[2,20]]
> levels btleft
[[20],[16],[8],[4],[2]]
```
有了这个，我们可以定义：
```haskell
treeBreadthFirst :: BT a -> [a]
treeBreadthFirst = concat . levels
```
其中 `.` 表示 function composition

<a name="gentree"></a>
# Inverting traversals 逆遍历，生成树


很多不同的 trees 可以有相同的 in-order / pre-order 

比如bt1` 和 `btleft`，有相同的 in-order

```hs
inOrder，preOder等等都是 :: BT a -> [a]
```
那么如果想构造一个 BT，使它具有给定的 in-order / pre-order

例如，下面这个 function 会根据一个 in-order traversal 产生一个 *balanced* BT：
l height 和 r height 的差最多是 1

如果 input 是 sorted 的，那么它会是一个 binary *search* tree【BST】：
```haskell
balancedTree :: [a] -> BT a
balancedTree [] = Empty
balancedTree xs = let (ys, x:zs) = splitAt (length xs `div` 2) xs in
                  Fork x (balancedTree ys) (balancedTree zs)
```
ghci自带的[`splitAt`]会在给定 position 把一个 list 分成两个 lists,用有pair输出
```
>splitAt 2 "hello"
("he","llo")
```
所有list都可以->tree然后再->回list
```hs
    treeInOrder (balancedTree xs) = xs
```
反过来，却不成立，因为BT变成list后，这个list可以代表不同形状的BT，从list再回BT就回不去了
```hs
    balancedTree (treeInOrder t) = t
```
并不是对所有 `t :: BT a` 都成立。例如：
```
     1                                                    2
    /                                                   /   \
   2    —treeInOrder—>  [3,2,1]  —treeInOrder—>        3     1
 /
3
```
## Balance把一棵树变平衡
其实就是像上面的图一样，先变list再变回tree
```haskell
balance :: BT a -> BT a
balance = balancedTree . treeInOrder
```
现在有了面的 `balancedTree` 方法，我们使用list comprehensions，就可以生成具有给定 in-order traversal 的 *all* binary trees 的 function

如inOrderTree [1,2,3]

意为：生成所有中序遍历结果为 [1,2,3] 的二叉树，上面的图已经是有两种结果了
```haskell
inOrderTree :: [a] -> [BT a]
inOrderTree [] = [Empty]
inOrderTree xs = [Fork x l r | i <- [0..length xs-1],
                               let (ys, x:zs) = splitAt i xs,
                               l <- inOrderTree ys, r <- inOrderTree zs]
```
它满足这个 property：
```hs
elem t (inOrderTree xs)--tree t 在 inOrderTree xs 生成出来的所有 trees 里面
```
当且仅当：
```hs
treeInOrder t = xs  --对 tree t 做 in-order traversal，结果刚好是 xs
```
对所有 `t :: BT a` 和 `xs :: [a]` 都成立。
例如，运行：
```hs
> inOrderTree [1..3]
[Fork 1 Empty (Fork 2 Empty (Fork 3 Empty Empty)),。。 (Fork 2 (Forkty) Empty]很多inOrder为123的tree
```
会成功计算出所有五个 in-order traversal 是 `[1,2,3]` 的 binary search trees：
```
   1
  / \
     2
    / \
       3
      / \
Fork 1 Empty (Fork 2 Empty (Fork 3 Empty Empty))

   1
  / \
     3
    / \
   2
  / \
Fork 1 Empty (Fork 3 (Fork 2 Empty Empty) Empty)

     2
    / \
   /   \
  1     3
 / \   / \
Fork 2 (Fork 1 Empty Empty) (Fork 3 Empty Empty)

    3
   / \
  1
 / \
    2
   / \
Fork 3 (Fork 1 Empty (Fork 2 Empty Empty)) Empty

      3
     / \
    2
   / \
  1
 / \
Fork 3 (Fork 2 (Fork 1 Empty Empty) Empty) Empty
```
## task
preOrderTree xs 生成所有 pre-order traversal 等于 xs 的 binary trees。。
```
preOrderTree :: [a] -> [BT a]
preOrderTree [] = [Empty]
preOrderTree (x:xs) =
  [Fork x l r | i <- [0..length xs],
                let (ys, zs) = splitAt i xs,
                l <- preOrderTree ys,
                r <- preOrderTree zs]
```


fuck you ↓
**Very hard task:** 写一个 function `breadthFirstTree :: [a] -> [BT a]`，满足这个 property：对所有 `t :: BT a` 和 `xs :: [a]`，`elem t (breadthFirstTree xs)` 当且仅当 `treeBreadthFirst t = xs`。[solution](https://patternsinfp.wordpress.com/2015/03/05/breadth-first-traversal/))

-------

下面是induction证明BT

.

.

.

.

.

.

.

.

.


.

.

.

.

.

.

。

。

。

。

。

.

<a name="bintreepf"></a>
## Proofs on binary trees by induction

如果我们有一个关于 trees 的 property `P`，并且想证明对所有 trees `t`，`P(t)` 都成立，那么可以用 *induction on trees*：

* 证明 `P(Empty)` 成立。
* 证明如果对给定 trees `l` 和 `r`，`P(l)` 和 `P(r)` 都成立，那么对 arbitrary `x`，`P(Fork x l r)` 也成立。

我们不会在这个 module 里特别强调 proofs，但当某些 claims 确实需要 proofs 时，我们会指出来。而且，我们会尽量精确地说明我们写的 programs 的 specifications。

经常会有这种情况：别人给我们展示了一个 clever algorithm，而我们因为不理解它而觉得自己很笨。但这种感觉是错的。如果我们不理解一个 algorithm，缺少的是 proof。Proof 就是 explanation。这就是 proof 的意思。为了理解一个 algorithm，我们需要：

  * algorithm 本身；
  * 它 intended to do 的 precise description；
  * 一个 convincing explanation，说明这个 algorithm 确实做到了它 intended to do 的事情。

Programs alone 是不够的。我们需要知道它们 intended to accomplish 什么，也想知道一个 explanation 来 justify 它们完成了我们 promise 的事情。这个 promise 叫做 algorithm / program 的 *specification*。Program correctness 意味着 “the promise is fulfilled”。尝试证明 promise 被 fulfilled 的一种方式是 *test* program。但实际上，testing 能做的只是通过找到 counterexamples 来说明 promise *not* fulfilled。当好的 examples 工作时，我们有某种 evidence 说明 algorithm works，但不是 full confidence，因为我们可能漏掉了会产生 wrong outputs 的 inputs。Full confidence 只能来自 convincing explanation，也就是 *proof*。如果你曾经问过自己 "proof" 到底是什么意思，ultimate answer 就是 "convincing argument"。

### Functional proofs

Dependently typed language [Agda](http://wiki.portal.chalmers.se/agda/pmwiki.php) 允许我们写 functional programs 以及它们的 correctness proofs，其中 [proofs themselves are written as functional programs](https://en.wikipedia.org/wiki/Curry%E2%80%93Howard_correspondence)。
例如，[这里有一个 computer-checked proof](http://www.cs.bham.ac.uk/~mhe/fp-learning-2017-2018/html/Agda-in-a-Hurry.html)，证明了上面 `isValid` 和 `validAddresses` 之间的 relation，在 Agda 里完成。
这部分不 examinable，只是为了 illustration。

**Task:** 写一个 function `preOrderTree :: [a] -> [BT a]`，满足这个 property：对所有 `t :: BT a` 和 `xs :: [a]`，`elem t (preOrderTree xs)` 当且仅当 `treePreOrder t = xs`。
```
preOrderTree :: [a] -> [BT a]
preOrderTree [] = [Empty]
preOrderTree (x:xs) =
  [Fork x l r | i <- [0..length xs],
                let (ys, zs) = splitAt i xs,
                l <- preOrderTree ys,
                r <- preOrderTree zs]
```

