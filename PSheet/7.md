...
instance Show a => Show (NumExpr a) where
  show e = showPrecExpr 0 e --当别人调用 show e 的时候，不直接在这里写复杂逻辑，而是交给 helper function：showPrecExpr。


showPrecExpr :: Show a => Int -> NumExpr a -> String
showPrecExpr p (Value x) =
  show x

showPrecExpr p (FromInteger n) =
  "fromInteger(" ++ show n ++ ")"

showPrecExpr p (Negate e) =
  "negate(" ++ showPrecExpr 0 e ++ ")"

showPrecExpr p (Abs e) =
  "abs(" ++ showPrecExpr 0 e ++ ")"

showPrecExpr p (SigNum e) =
  "signum(" ++ showPrecExpr 0 e ++ ")"
...
eval env (Var nm) = lookupEnv env nm
...
eval (Var nm) = error ("Variable has no value: " ++ nm)
...
```haskell
cycles :: Perm -> [[Int]]
cycles p = go [0 .. length p - 1]
  where
    go [] = []
    go (x:xs)
      | alreadySeen x = go xs
      | otherwise     = c : go remaining
      where
        c = cycleOf x p

        alreadySeen n =
          any (n `elem`) (goBefore p [0 .. x - 1])

        remaining =
          [y | y <- xs, not (y `elem` c)]


goBefore :: Perm -> [Int] -> [[Int]]
goBefore p [] = []
goBefore p (x:xs) =
  cycleOf x p : goBefore p xs
```
...
cycles :: Perm -> [[Int]]
cycles p = reverse (go [0 .. length p - 1] [])
  where
    go [] seenCycles = seenCycles

    go (x:xs) seenCycles
      | x `elem` concat seenCycles = go xs seenCycles
      | otherwise                  = go xs (cycleOf x p : seenCycles)
```
