...
applyfuns f g (Fork l x r) = 
 (Fork (applyfuns f g l) (f x) (applyfuns f g l))
```
### Examples
...
updateNodes [] f t = t
updateNodes _ f Empty = Empty
updateNodes (GoLeft:ys) f (Node l x r) = 
 Node (updateNodes ys f l) (f x) r
 
updateNodes (GoRight:ys) f (Node l x r) = 
 Node l (f x) (updateNodes ys f r)
```
