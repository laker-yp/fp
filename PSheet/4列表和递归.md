...
- [List Comprehensions](#list-comprehensions)
  - [答案：Pythagorean triples 和 Perfect numbers](#答案pythagorean-triples-和-perfect-numbers)
  - [Variations：Abundant / Deficient / Highly Abundant](#variationsabundant--deficient--highly-abundant)
...
- [Recursive Functions](#recursive-functions)
  ...
  - [`elem'`](#elem)
  - [`merge`](#merge)
- [List Comprehensions and Higher-order Functions](#list-comprehensions-and-higher-order-functions)
  - [答案：`map` / `filter` with `foldr` and `foldl`](#答案map--filter-with-foldr-and-foldl)
  - [Church Numerals](#church-numerals)
- [Defining the Prelude `concat` Function in Four Different Ways](#defining-the-prelude-concat-function-in-four-different-ways)
  - [答案：`concat1` / `concat2` / `concat3`](#答案concat1--concat2--concat3)
- [Testing Your `concat` Functions](#testing-your-concat-functions)
- [99 Haskell Problems](#99-haskell-problems)
...
### 答案：Pythagorean triples 和 Perfect numbers
...
### Variations：Abundant / Deficient / Highly Abundant
...
- A number which is greater than the sum of its proper divisors is called [deficient](https://en.wikipedia.org/wiki/Deficient_number).
...
4. **Harder** Implement [matrix multiplication](https://en.wikipedia.org/wiki/Matrix_multiplication), where matrices are represented by lists of lists of integers. One possibility would be to take the dimensions of the matrices as arguments, so that your function would have type:
...
2. Redefine `map f` and `filter p` using `foldr` and `foldl`. For your reference, here are the definitions of `map` and `filter` from lecture notes.
...
### 答案：`map` / `filter` with `foldr` and `foldl`
...
These are called **Church Numerals**. The encoding works like this: the input to a function of the above type is an element `f` of type `a -> a`. Since such a function takes input values of type `a` and also produces output values of type `a`, this means it can be iterated. So we represent the numeral `n` by the element of type `(a -> a) -> (a -> a)` which iterates its argument `n` times.

To see the idea, the first few examples of numerals are written like this:
...
1. Define it with comprehensions and no recursion.

   Hint: You will need two generators, one to extract a list `xs` from the list of lists `xss`, and another to extract an element `x` from the list `xs`, and put this in the result of the comprehension.

   ```hs
   concat1 :: [[a]] -> [a]
   ```

2. Define the same function using recursion instead.

   Hint: Find and use the prelude function `++`.

   ```hs
   concat2 :: [[a]] -> [a]
   ```

3. Define the same function using `foldr` and `foldl`, and without recursion or list comprehensions.
...
3. Better run-time test. Check how time increases when `n` increases in the following tests:
